...
eonia_curve_c = ql.PiecewiseLogCubicDiscount(0,
                                             ql.TARGET(),
                                             helpers,
                                             ql.Actual365Fixed())
...
dts = [datetime.datetime(d.year(), d.month(), d.dayOfMonth()) for d in dates]

plt.plot(dts, rates_c)
plt.xlabel('Time')
plt.ylabel('YieldCurve')
plt.title('YieldCurve Curve')
plt.show()